...
function photoway_scripts(){
	...
	$scripts = array(
		array(
			...
			'style'    => esc_url( 'https://fonts.googleapis.com/css?family=Hind:300,400,400i,500,600,700,800,900|Playfair+Display:400,400italic,700,900'),
			'absolute' => true
		),
		array(
			'handler' => 'bootstrap',
...
* @since Photoway 1.0.9
* @return null
*/
function photoway_ocdi_after_import_setup() {
...
add_action( 'pt-ocdi/after_import', 'photoway_ocdi_after_import_setup' );
...
function photoway_exclude_single_posts_home($query) {
    if( !is_single() ){
    	$query->set('post__not_in', get_option( 'sticky_posts' ));
    }
}

add_action('pre_get_posts', 'photoway_exclude_single_posts_home');
